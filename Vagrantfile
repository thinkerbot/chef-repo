...
hosts_file = File.expand_path("../vm/config/hosts", __FILE__)
hosts_data = File.read(hosts_file)
hosts = Hash[hosts_data.split("\n").map {|line| line.split(/\s+/, 2).reverse }]
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "chef-server" do |server|
    ...
    server.vm.hostname = "chef-server.example.com"
    server.vm.network :private_network, ip: hosts["chef-server.example.com"]
    server.vm.provision :shell, :path => "vm/server/setup.sh"
  ...
  config.vm.define "chef-workstation" do |workstation|
    workstation.vm.box = "precise64"
    workstation.vm.box_url = "http://files.vagrantup.com/precise64.box"
